...
import saetabis.automation.testingWrapper.services.ui.common.ResourceNotFoundException;
...
public class BasePage {
    ...
    public void selectDropDownByValue(@NotNull By by, @NotNull String text) {
    ...
    public void changeDriverContextToWeb(WebDriver driver) {
        log.info("Change driver context to : webview");
        pause(5);
        ...
        availableContexts.stream()
                ...
                .forEach(((AppiumDriver<?>) driver)::context);
    }

    public static void changeDriverContextToNative(WebDriver driver) {
        if (!((AppiumDriver<?>) driver).getContext().toLowerCase().contains("native")) {
            log.info("Change driver context to : native app");
    ...
    public <T extends BasePage> T refreshPage(Class<T> page) {
        driver.navigate().refresh();
        return getPage(page);
    }

    public <T extends BasePage> T refreshPage( T page) {
        driver.navigate().refresh();
        return page;
    }
